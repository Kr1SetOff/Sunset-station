...
using Content.Shared._Sunset.Bitrunning.Prototypes;
using Content.Shared._Sunset.Bitrunning.Systems;
...
namespace Content.Server._Sunset.Bitrunning.Systems;
...
public sealed class QuantumServerSystem : EntitySystem
{
    ...
    // 🌇Sunset🌇 - dropped the RefreshPartsEvent/UpgradeExamineEvent-driven machine-part upgrade hooks
    // (Content.Shared._Orion.Construction doesn't exist in this fork). ScannerTier/CooldownMultiplier/
    // QualityBonus/FinalExitDamageMultiplier just stay at their component defaults (tier 1, unupgraded).
    ...
    private void SpawnObjectiveMarkers(List<(EntProtoId Prototype, EntityCoordinates Coordinates)> markerCandidates, bool isModular, int objectiveTarget)
    {
        ...
        if (!isModular)
        {
            foreach (var marker in markerCandidates)
            {
                ...
                Spawn("EffectSparks", marker.Coordinates);
            }
    ...
    private void SetAvatarBroadcastEnabled(Entity<AvatarConnectionComponent> avatar, QuantumServerComponent server, bool enabled)
    {
        ...
        // 🌇Sunset🌇 - DeviceNetworkComponent.NetIdEnum is write-restricted to SharedDeviceNetworkSystem
        // in this fork (no public setter), so it's left at its default net ID; and
        // SurveillanceCameraSystem has no ConfigureCameraNetwork - ensuring the component is enough
        // for the avatar to show up as a generic camera on the network.
        EnsureComp<DeviceNetworkComponent>(cameraEntity);
    ...
    public void DisconnectAvatar(EntityUid avatarUid, bool harmful)
    {
        ...
        {
            // 🌇Sunset🌇 - SurveillanceCameraSystem in this fork has no ClearActiveViewers; viewers
            // naturally lose the feed once SurveillanceCameraComponent is removed from the avatar above.
    ...
    // 🌇Sunset🌇 - GetAntagSelectionBlockerEvent doesn't exist in this fork; AntagImmuneComponent
    // (already applied in SpawnAvatarForRunner below) already keeps avatars out of antag selection.
    ...
    // 🌇Sunset🌇 - always spawns the generic AvatarPrototype: the player-profile-appearance avatar
    // feature (BitrunningProfileDiskComponent, ShouldLoadProfileAvatar/TryGetHumanoidProfile) needed
    // Content.Server._Orion.CloningAppearance.Systems.CloningAppearanceSystem, which doesn't exist in
    // this fork and wasn't otherwise in scope to port.
    ...
    private void StripNonAvatarPolymorphComponents(EntityUid avatar)
    {
        ...
        // 🌇Sunset🌇 - FaunaComponent (Content.Shared._Lavaland.Mobs) doesn't exist in this fork.
    }
