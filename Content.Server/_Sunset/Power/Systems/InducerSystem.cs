...
using Content.Shared._Sunset.Power.Components;
...
namespace Content.Server._Sunset.Power.Systems;
...
public sealed class InducerSystem : EntitySystem
{
    ...
    private void OnDoAfter(EntityUid uid, InducerComponent component, InducerDoAfterEvent args)
    {
        ...
        baseEnergyToConsume = Math.Min(baseEnergyToConsume, _battery.GetCharge((slot.Item.Value, sourceBattery)));

        if (baseEnergyToConsume <= 0)
            return;

        var energyToReceive = baseEnergyToConsume * effectiveMultiplier;
        var freeSpace = targetBattery.MaxCharge - _battery.GetCharge((target, targetBattery));
        ...
        {
            _battery.SetCharge((target, targetBattery), _battery.GetCharge((target, targetBattery)) + energyToReceive);
            _battery.SetCharge((slot.Item.Value, sourceBattery), _battery.GetCharge((slot.Item.Value, sourceBattery)) - actualEnergyToConsume);
            args.Repeat = false;
        }
    }
