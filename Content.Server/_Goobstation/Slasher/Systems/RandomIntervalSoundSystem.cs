    ...
    public override void Update(float frameTime)
    {
        ...
        while (query.MoveNext(out var uid, out var comp))
        {
            ...
            if (comp.RequireEquipped)
            {
                ...
                else if (_containers.TryGetContainingContainer((uid, null, null), out var container))
                    owner = container.Owner;
            }

            if (comp.RequireAlive)
            {
                if (owner == null && _containers.TryGetContainingContainer((uid, null, null), out var container))
                    owner = container.Owner;
