...
public sealed class SlasherIncorporealCameraSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<SlasherIncorporealComponent, SlasherIncorporealExitedEvent>(OnIncorporealExited);
    ...
    private void OnIncorporealEntered(Entity<SlasherIncorporealComponent> ent, ref SlasherIncorporealEnteredEvent args)
    {
        ...
        // Pressure immunity only exists server-side in this fork, so it's applied here.
        EnsureComp<PressureImmunityComponent>(ent);

        DisableLightsInArea(ent);
    }

    private void OnIncorporealExited(Entity<SlasherIncorporealComponent> ent, ref SlasherIncorporealExitedEvent args)
    {
        RemCompDeferred<PressureImmunityComponent>(ent);
    }
