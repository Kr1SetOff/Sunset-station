...
using Content.Shared._Goobstation.InternalResources.Components;
...
public sealed partial class ChangelingSystem
{
    ...
    private void OnShriekResonant(EntityUid uid, ChangelingIdentityComponent comp, ref ShriekResonantEvent args)
    {
        ...
        var power = comp.ShriekPower;
        var lights = GetEntityQuery<Content.Shared.Light.Components.PoweredLightComponent>();
    ...
    private void OnStingFakeArmblade(EntityUid uid, ChangelingIdentityComponent comp, ref StingFakeArmbladeEvent args)
    {
        ...
        if (TryComp<HandsComponent>(target, out var handComp)
            && handsValid)
        {
            var weaponCount = _hands.EnumerateHeld((target, handComp)).Count(e => HasComp<ChangelingFakeWeaponComponent>(e));
    ...
    public void OnBiodegrade(EntityUid uid, ChangelingIdentityComponent comp, ref ActionBiodegradeEvent args)
    {
        ...
        if (_pull.IsPulled(uid))
        {
            ...
            if (puller != null)
            {
                ...
                _stun.TryKnockdown(puller.Value, TimeSpan.FromSeconds(1.5), refresh: true);
    ...
    public void OnHivemindAccess(EntityUid uid, ChangelingIdentityComponent comp, ref ActionHivemindAccessEvent args)
    {
        ...
        // Adapted for this fork: hivemind chat uses the Language/radio system (matches how
        // other telepathic antags - Dantalion, Abductors, etc. - implement it here) rather
        // than Goob-Station's CollectiveMind component, which this fork doesn't have.
