...
public sealed partial class ChangelingSystem : SharedChangelingSystem
{
    ...
    {
        // Adapted for this fork: FlashImmunityComponent.Enabled is only writable by
        // SharedFlashSystem, and there's no public toggle for it, so immunity is
        // represented by presence/absence of the component instead.
        if (active)
    ...
    private void UpdateAbilities(EntityUid uid, ChangelingIdentityComponent comp)
    {
        ...
        if (comp.StrainedMusclesActive)
        {
            ...
            _stamina.TakeStaminaDamage(uid, 7.5f, stamina, visual: false);
    ...
    {
        return TryComp<GrabbableComponent>(uid, out var grabbable) && grabbable.Stage > GrabStage.Passive;
    }
        ...
        bool persistentDna = false)
    {
        ...
        // Adapted for this fork: upstream Goob-Station looped over a List<Type> calling
        // nameof(type), which always evaluated to the literal string "type" rather than the
