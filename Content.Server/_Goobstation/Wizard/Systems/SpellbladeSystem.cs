...
public sealed class SpellbladeSystem : SharedSpellbladeSystem
{
    ...
    public override void Update(float frameTime)
    {
        ...
        foreach (var (uid, temporal, damageable, xform) in toDamage)
        {
            ...
            // Sunset - this fork's TryChangeDamage has no targetPart (Shitmed) parameter, so the temporal
            // slash just applies to the entity's overall damage container instead of a specific body part.
            _damageable.TryChangeDamage((uid, damageable), temporal.Damage);
    ...
    private void OnLightningHit(Entity<LightningSpellbladeEnchantmentComponent> ent, ref MeleeHitEvent args)
    {
        ...
        // Sunset - this fork's LightningSystem.ShootRandomLightnings has no ignoredEntity/per-bolt callback
        // overload like Goob's does, so we can't stamp each bolt with the enchantment's ShockDamage/ShockTime/
        // Siemens values or make bolts ignore the wielder. Bolts fall back to LightningSystem's own defaults.
