...
using Content.Shared._Goobstation.Wizard.RandomTeleport;
...
public sealed class SpellsSystem : SharedSpellsSystem
{
    ...
    // Adapted for this fork: this fork's own Wizard antag rule already exists (grants
    // Content.Shared.Roles.Components.WizardRoleComponent, mind role "MindRoleWizard", faction
    // "Wizard") - Goob-Station's own WizardRuleSystem/apprentice framework wasn't ported (out of
    // scope - the ask was to add spells to the already-existing grimoire, not replace the antag
    // rule), so the static WizardRuleSystem.Faction/Role.Id references are replaced with this
    // fork's actual antag wiring here.
    protected override void BindSoul(BindSoulEvent ev, EntityUid item, EntityUid mind, MindComponent mindComponent)
    {
        ...
        Faction.AddFaction(newEntity, "Wizard");
        ...
        if (!Role.MindHasRole<GoobWizardRoleComponent>(mind, out _))
            Role.MindAddRole(mind, "MindRoleWizard", mindComponent, true);
    ...
    // Adapted for this fork: dropped the Shitmed-only healing-split (targetPart/splitDamage) - this
    // fork's DamageableSystem.TryChangeDamage has no equivalent parameters, so it heals as a single
    // pool change instead of being split across organic limbs.
    public override void SpeakSpell(EntityUid speakerUid, EntityUid casterUid, string speech, MagicSchool school)
    {
        ...
        if (Exists(casterUid))
        {
            ...
            {
                Damageable.TryChangeDamage(casterUid, -invocationEv.ToHeal, true, false);
