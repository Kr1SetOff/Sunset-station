...
public sealed class WizardMirrorSystem : SharedWizardMirrorSystem
{
        ...
        HumanoidAppearanceComponent humanoid)
    {
            ...
            _proto.TryIndex(profile.Species, out var speciesProto))
        {
            ...
            // Sunset - SlaughterDemon/BloodCrawl was never ported to this fork (it's outside the wizard
            // equipment-item port scope), so there's no BloodCrawlComponent to block species changes for.
            ...
            var config = new PolymorphConfiguration
            {
                ...
                RevertOnCrit = false,
                RevertOnDeath = false,
                // Sunset - this fork's PolymorphConfiguration exposes a flat HashSet<string> of component
                // names (CopiedComponents) instead of Goob's ComponentsToTransfer list of (name, mirror)
                // entries, so the per-component "mirror" flag isn't representable here.
                CopiedComponents = new()
                {
                    "LanguageKnowledge",
                    "LanguageSpeaker",
                    "Grammar",
        ...
        foreach (var marking in profile.Appearance.Markings)
        {
            ...
            {
                // Sunset - AddMarking's (IReadOnlyList<Color>) overload gained an extra leading
                // isGlowing parameter compared to Goob's, so sync must now be passed by name.
                _humanoid.AddMarking(target, marking.MarkingId, marking.MarkingColors, false, sync: false);
            }
            else
            {
        ...
        {
            _humanoid.AddMarking(target, profile.Appearance.HairStyleId, false, hairColor, sync: false);
        }

        if (_markingManager.Markings.TryGetValue(profile.Appearance.FacialHairStyleId, out var facialHairPrototype) &&
            _markingManager.CanBeApplied(profile.Species, profile.Sex, facialHairPrototype, _proto))
        {
            _humanoid.AddMarking(target, profile.Appearance.FacialHairStyleId, false, facialHairColor, sync: false);
        }
        ...
        foreach (var (marking, prototype) in markingFColored)
        {
            ...
            _humanoid.AddMarking(target, marking.MarkingId, markingColors, false, sync: false);
        }
        ...
        // Sunset - Content.Shared._Shitmed.Humanoid.Events.ProfileLoadFinishedEvent doesn't exist on this
        // fork (no Shitmed body-part system to react to it), so this notification is simply dropped.
