...
/// <summary>
/// НОВАЯ МЕХАНИКА «Биокод». Привязывает предмет к одному конкретному ТЕЛУ (кукле персонажа)
/// прямо во время игры (через ПКМ-верб «Установить биокод»). Пока биокод не установлен
/// (<see cref="OwnerEntity"/> == null), предмет общедоступен. После установки:
///  - предметами-инструментами (аплинк, оружие, снаряжение) может пользоваться только кукла-владелец
///    (блокируются открытие UI, использование в руке, стрельба; чужаку — попап и сообщение в чат);
///  - предметы-скафандры (<see cref="DetonateOnForeignWear"/> == true), надетые ЧУЖИМ,
...
{
    /// <summary>
    /// Кукла-владелец биокода (само тело персонажа, а не аккаунт игрока — по РП биокод считывает
    /// ДНК тела). null — биокод ещё не установлен, предмет общедоступен.
