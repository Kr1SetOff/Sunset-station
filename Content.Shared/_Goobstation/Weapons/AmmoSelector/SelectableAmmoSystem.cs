...
public sealed class SelectableAmmoSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<AmmoSelectorComponent, GunRefreshModifiersEvent>(OnRefreshModifiers);
    }

    // Adapted for this fork: GunComponent.SoundGunshot/FireRate are only writable by
    // SharedGunSystem itself here, so the selected ammo's sound/fire rate is applied
    // through the modifier-refresh event pipeline instead of setting the base fields directly.
    private void OnRefreshModifiers(Entity<AmmoSelectorComponent> ent, ref GunRefreshModifiersEvent args)
    {
        if (ent.Comp.CurrentlySelected is not { } selected
            || !_protoManager.TryIndex(selected, out var index))
            return;

        if (ShouldSetSound(index))
            args.SoundGunshot = index.SoundGunshot;

        if (ShouldSetFireRate(index))
            args.FireRate = index.FireRate;
    }
    ...
    public bool TrySetProto(Entity<AmmoSelectorComponent> ent, ProtoId<SelectableAmmoPrototype> proto)
    {
        ...
        if ((ShouldSetSound(index) || ShouldSetFireRate(index)) && TryComp(ent, out GunComponent? gun))
