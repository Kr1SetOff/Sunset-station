...
public abstract class SharedSpellsSystem : EntitySystem
{
    ...
    private void OnCorpseExplosion(CorpseExplosionEvent ev)
    {
        ...
        foreach (var (target, damageable) in targets)
        {
            ...
            if (HasComp<BorgChassisComponent>(target))
                ...
                Stun.TryKnockdown(target, ev.KnockdownTime / range, refresh: true);
        }
    ...
    private void OnSoulTap(SoulTapEvent ev)
    {
        ...
        if (kill)
        {
            ...
            var success = Damageable.TryChangeDamage(ev.Performer,
                new DamageSpecifier(ProtoMan.Index(ev.KillDamage), 666),
                out var dmg,
                true);
            if ((!success || dmg.GetTotal() < 1) && Timing.IsFirstTimePredicted)
                _gibbing.Gib(ev.Performer);
        }
    ...
    private void StopPullingHelper(EntityUid uid)
    {
        if (TryComp<PullerComponent>(uid, out var puller) && puller.Pulling is { } pulling
            ...
            _pulling.TryStopPull(pulling, pullable);

        if (TryComp<PullableComponent>(uid, out var selfPullable))
            _pulling.TryStopPull(uid, selfPullable);
    }
    ...
    {
        StopPullingHelper(uid);
        StopPullingHelper(otherUid);
