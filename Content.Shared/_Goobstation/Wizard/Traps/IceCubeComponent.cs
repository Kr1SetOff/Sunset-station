...
// 🌇Sunset🌇 - Goobstation's IceCubeComponent inherits Content.Shared._Shitcode.Heretic.SpriteOverlay's
// BaseSpriteOverlayComponent, which is part of the Heretic antag's client-side layered sprite overlay
// framework. That framework isn't ported to this fork, so this is now a plain component with the same
// data fields, minus the client overlay wiring - the ice-cube visual can be hooked up later via a
// dedicated appearance/sprite layer system if desired.
...
public sealed partial class IceCubeComponent : Component
{
    ...
    public Enum Key { get; set; } = IceCubeKey.Key;
