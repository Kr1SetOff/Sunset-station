...
public abstract class SharedChangelingBiomassSystem : EntitySystem
{
    ...
    private void KillChangeling(Entity<ChangelingBiomassComponent> ent)
    {
        ...
        // Shitmed substitution: no body-part targeting available, so the full lethal amount is
        // applied directly to the entity's overall damage pool instead of being split across organic limbs.
        var damagespec = new DamageSpecifier(genetic, (FixedPoint2) totalDamage);
        _dmg.TryChangeDamage(ent.Owner, damagespec, ignoreResistances: true, origin: ent.Owner);
