...
public abstract partial class SharedChangelingStasisSystem : EntitySystem
{
    ...
    private void ExitStasis(Entity<ChangelingStasisComponent> ent, bool heal = true)
    {
           ...
           && _pull.IsPulled(ent, pullComp))
        {
            ...
            if (puller != null)
                _stun.TryAddParalyzeDuration(puller.Value, ent.Comp.StunTime);
        }
    ...
    private void RestoreChangeling(Entity<ChangelingStasisComponent> ent)
    {
            ...
            && _dmgQuery.TryComp(ent, out var dmgComp))
        {
            ...
            _dmg.SetAllDamage((ent.Owner, dmgComp), 0);
            _mob.SetAllowRevives(ent, false, threshComp);
        }

        // TODO: Shitmed substitution - Sunset has no surgery/wounds system (Content.Shared._Shitmed),
        // so trauma/broken-bone repair from the original Goob-Station code was dropped here. The overall
        // damage reset above already fully heals the changeling; only body-part-specific trauma/bone
        // healing (which has no plain-damage equivalent) is skipped.
