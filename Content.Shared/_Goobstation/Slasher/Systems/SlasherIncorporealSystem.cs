...
/// <summary>
/// Slasher incorporeal (jaunt) form.
/// Goob-Station delegates the actual phasing to its PhaseShiftedComponent; that system does not exist in
/// this fork, so the phasing itself (collision, stealth, speed, effects) is handled inline here.
/// </summary>
public sealed class SlasherIncorporealSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<SlasherIncorporealComponent, RefreshMovementSpeedModifiersEvent>(OnRefreshSpeed);
    ...
    public void EnterIncorporeal(EntityUid uid, Entity<SlasherIncorporealComponent> ent)
    {
        ...
        // Phase out: drop collision so we can pass through walls.
        SetPhased(uid, ent.Comp, true);

        if (_net.IsServer)
        {
            Spawn(ent.Comp.JauntInEffect, _transform.GetMapCoordinates(uid));
            _audio.PlayPvs(ent.Comp.JauntDisappear, uid);
        }
        ...
        // Space / environment immunity
        EnsureTrackedComp<MovementIgnoreGravityComponent>(uid, ent);
        EnsureTrackedComp<TemperatureImmunityComponent>(uid, ent);
    ...
    public void ExitIncorporeal(EntityUid uid, Entity<SlasherIncorporealComponent> ent)
    {
        ...
        SetPhased(uid, ent.Comp, false);

        if (_net.IsServer)
        {
            Spawn(ent.Comp.JauntOutEffect, _transform.GetMapCoordinates(uid));
            _audio.PlayPvs(ent.Comp.JauntAppear, uid);
        }
        ...
        var exitedEv = new SlasherIncorporealExitedEvent();
        RaiseLocalEvent(uid, ref exitedEv);
    }

    /// <summary>
    /// Turns the slasher's collision on/off so they can walk through walls while incorporeal.
    /// </summary>
    private void SetPhased(EntityUid uid, SlasherIncorporealComponent comp, bool phased)
    {
        ...
        if (phased)
        {
            comp.StoredCollisionMask = fixture.Value.CollisionMask;
            comp.StoredCollisionLayer = fixture.Value.CollisionLayer;
        ...
        {
            _physics.SetCollisionMask(uid, fixture.Key, fixture.Value, comp.StoredCollisionMask, fixtures);
            _physics.SetCollisionLayer(uid, fixture.Key, fixture.Value, comp.StoredCollisionLayer, fixtures);
        }
    }

    private void OnRefreshSpeed(Entity<SlasherIncorporealComponent> ent, ref RefreshMovementSpeedModifiersEvent args)
    {
        if (!ent.Comp.IsIncorporeal)
            return;

        args.ModifySpeed(ent.Comp.IncorporealSpeedBuff, ent.Comp.IncorporealSpeedBuff);
    }
    ...
    private void OnBeforeDamage(Entity<SlasherIncorporealComponent> ent, ref BeforeDamageChangedEvent args)
    {
        if (ent.Comp.IsIncorporeal)
            args.Cancelled = true;
    }

    private void OnGettingAttackedAttempt(Entity<SlasherIncorporealComponent> ent, ref GettingAttackedAttemptEvent args)
    {
        if (ent.Comp.IsIncorporeal)
            args.Cancelled = true;
    }
