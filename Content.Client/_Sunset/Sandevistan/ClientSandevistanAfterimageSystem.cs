...
/// <summary>
/// Gives each server-spawned (bare, prototype-less) SandevistanAfterimageComponent entity a
/// client-only copy of the source's sprite, rainbow-tinted and locked to the direction it was
/// facing at spawn time.
/// </summary>
public sealed class ClientSandevistanAfterimageSystem : EntitySystem
{
    ...
    private void OnAfterimageStartup(Entity<SandevistanAfterimageComponent> ent, ref ComponentStartup args)
    {
        ...
        // This fork's DrawDepth enum has no "FloorEffects" tier - FloorObjects is the closest match
        // (things resting right on the floor, below mobs/items).
        _sprite.SetDrawDepth((ent.Owner, afterimageSprite), (int) DrawDepthEnum.FloorObjects);
